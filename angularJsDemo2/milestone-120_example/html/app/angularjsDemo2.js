...
app.controller('adminHomeController',  function($scope, $http, $mdToast, $rootScope){
});
app.controller('userListController',  function($scope, $http, $mdToast, $rootScope){
});


/*Xử lý angular phần add user*/
app.controller('userAddController',  function($scope, $http, $mdToast, $rootScope){	
			...
			.then(function(res) {
				...
				if (res.data.emailLogin==$scope.userSignIn.emailDemo2) {	/*tính này bằng vs emailDemo2 từ view nhập vào thì success*/
					if(res.data.adminLogin==1){		/*nếu thuộc tính adminLogin trong res.data =1 thì là admin. Ngược lại thì là user*/
						...
						console.log($rootScope.appDemo2);
					}
					else{
...
app.controller('AngularJsControllerDemo2',  function($scope, $http, $mdToast, $rootScope){
});
